...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(true);
    try {
      // Redireciona de volta para a raiz; o AppRoutes decide o destino conforme o acesso.
      ...
      if (err) throw err;
      // O redirect é gerenciado pelo fluxo OAuth do Supabase; nada a fazer aqui.
    } catch (err) {
      ...
      <div className="w-full max-w-md bg-card rounded-2xl shadow-xl p-8 border border-border">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Entre com sua conta Young para continuar
          </p>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 px-4 rounded-lg border border-input bg-background text-foreground font-medium hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 flex items-center justify-center gap-3"
        >
          {loading ? (
