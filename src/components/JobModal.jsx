...
const JobModal = ({ isOpen, job, onClose, onSave, options, isSaving, candidates = [], isPageMode = false }) => {
    ...
    const formContent = (
        <>
            <div className={`px-6 py-4 border-b border-border flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 ${isPageMode ? 'rounded-t-xl' : ''}`}>
                <h3 className="font-bold text-xl text-foreground">{d.id ? 'Editar' : 'Nova'} Vaga</h3>
                <button onClick={onClose} className="text-muted-foreground hover:text-gray-700 dark:hover:text-white flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                ...
                <div className="space-y-4">
                    <h4 className="text-sm font-semibold text-muted-foreground border-b border-border pb-2">Informações Principais</h4>

                    {/* Título / Nome da vaga */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Título da vaga</label>
                        <input
                            type="text"
                            className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            placeholder="Ex.: Cargo - Empresa"
                            value={d.title || ''}
                            onChange={e => setD({ ...d, title: e.target.value })}
                        />
                        <p className="text-xs text-muted-foreground mt-1">Nome exibido na lista de vagas. Se vazio, será gerado a partir do cargo e empresa ao salvar.</p>
                    </div>

                    {/* Empresa */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Empresa *</label>
                        <div className="flex gap-2">
                            <select
                                className="flex-1 bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                value={d.company || ''}
                        ...
                        {showNewCompany && (
                            <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-900 border border-border rounded-lg space-y-2">
                                <input
                                    type="text"
                                    className="w-full bg-card border border-border p-2 rounded text-sm text-foreground outline-none focus:border-blue-500"
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => setShowNewCompany(false)} className="px-3 py-1.5 text-muted-foreground text-sm hover:text-gray-700 dark:hover:text-gray-200">Cancelar</button>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        {showNewCity && (
                            <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-900 border border-border rounded-lg space-y-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => setShowNewCity(false)} className="px-3 py-1.5 text-muted-foreground text-sm hover:text-gray-700 dark:hover:text-gray-200">Cancelar</button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Setor (multi-seleção) */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Setor</label>
                        ...
                        {showNewSector && (
                            <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-900 border border-border rounded-lg space-y-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => setShowNewSector(false)} className="px-3 py-1.5 text-muted-foreground text-sm hover:text-gray-700 dark:hover:text-gray-200">Cancelar</button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Cargo (multi-seleção) */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Cargo</label>
                        <div className="flex flex-wrap gap-x-4 gap-y-2 items-center">
                            {positions.map(p => {
                        ...
                        {showNewPosition && (
                            <div className="mt-2 p-3 bg-gray-50 dark:bg-gray-900 border border-border rounded-lg space-y-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button onClick={() => setShowNewPosition(false)} className="px-3 py-1.5 text-muted-foreground text-sm hover:text-gray-700 dark:hover:text-gray-200">Cancelar</button>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* Prazo */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Prazo para Preenchimento</label>
                        <input
                            type="date"
                            className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            value={d.deadline || ''}
                    ...
                    <div>
                        ...
                        <textarea
                            className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 h-24 resize-none"
                    ...
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Recrutador Responsável</label>
                        <select
                            className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            value={d.recruiter || ''}
                    ...
                    {(d.id && d.requestedByUserId) && (
                        <div>
                            <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Quem solicitou a abertura</label>
                            <p className="text-sm text-muted-foreground bg-muted/50/50 border border-border rounded-lg px-2.5 py-2">
                                {(() => {
                            ...
                            <p className="text-xs text-muted-foreground mt-1">Usuário que criou a abertura da vaga no sistema (registrado automaticamente).</p>
                        </div>
                    )}

                    {/* Quem autorizou a abertura */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Quem autorizou a abertura</label>
                        <select
                            className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            value={d.approvedBy || ''}
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Responsável pela solicitação e aprovação da vaga (não quem abre no sistema).</p>
                    </div>

                    {/* Onde a vaga será divulgada */}
                    <div>
                        <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Onde a vaga será divulgada</label>
                                    ...
                                    <div key={ch.id} className="flex items-center gap-2 flex-wrap">
                                        ...
                                        {ch.hasCustomText && isChecked && (
                                            <input
                                                ...
                                                className="ml-1 w-32 bg-background border border-border px-2 py-1 rounded text-sm text-foreground outline-none focus:border-blue-500"
                ...
                <div className="border-t border-border pt-4">
                    <button
                        ...
                        className="flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-gray-900 dark:hover:text-white transition-colors"
                    >
                    ...
                    {showOptionalFields && (
                        <div className="mt-4 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Código da Vaga</label>
                                    <input
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Função</label>
                                    <select
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                        value={d.function || ''}
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Nº de Vagas</label>
                                    <input
                                        type="number"
                                        min="1"
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Prioridade</label>
                                    <select
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Tipo de Contrato</label>
                                    <select
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Modelo de Trabalho</label>
                                    <select
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Carga Horária</label>
                                    <input
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Faixa Salarial</label>
                                    <input
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Gestor Contratante</label>
                                    <input
                                        className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Requisitos</label>
                                <textarea
                                    className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 h-24 resize-none"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5">Benefícios</label>
                                <textarea
                                    className="w-full bg-background border border-border p-2.5 rounded-lg text-sm text-foreground outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 h-20 resize-none"
            ...
            <div className="px-6 py-4 border-t border-border flex justify-end gap-2 bg-gray-50 dark:bg-gray-900/50">
                <button onClick={onClose} className="px-6 py-2 text-muted-foreground hover:text-gray-900 dark:hover:text-white transition-colors">Cancelar</button>
                <button
                    onClick={() => {
    ...
    if (isPageMode) {
        return (
            <div className="p-6 bg-background min-h-full">
                <div className="max-w-2xl mx-auto bg-card rounded-lg border border-border flex flex-col shadow-sm">
                    {formContent}
                </div>
            </div>
        );
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
            <div className="bg-card rounded-lg w-full max-w-2xl max-h-[90vh] border border-border flex flex-col shadow-2xl">
                {formContent}
            </div>
        </div>
    );
